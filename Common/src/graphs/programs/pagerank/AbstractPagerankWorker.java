...
public abstract class AbstractPagerankWorker extends GraphWorker<GraphVertex,GraphEdge> {
	...
	protected void performAction(ChannelElement channelElement) {	
		if (channelElement instanceof MRChannelElement<?, ?>) {
		...
		else if(channelElement instanceof MarkerChannelElement)
		{
			performSpecialAction((MarkerChannelElement)channelElement);
		}
			
	}

	protected abstract void performSpecialAction(MarkerChannelElement element);
...
//					boolean ret = write(new MRChannelElement<Integer, Double>(Integer.valueOf(foreignEdge.getTargetName()),
//							pagerankContribution), ownerWorker);
		...
		{
			write(new MarkerChannelElement(null), channelName);
		}
	...
	@SuppressWarnings("unchecked")
	protected
	final MRChannelElement<Integer, Double> readPagerankChannelElement()
	{	
		return (MRChannelElement<Integer, Double>) read(pagerankInputFilename);
	}
	...
	protected boolean loadCache() {
		...
		try {
			...
			if (cache != null) {
				 ...
				 */
				MRChannelElement<Integer, Double> element = null;

				while ((element = readPagerankChannelElement()) != null) {
	...
	protected void loadGraph() {
		...
		while (true) {
			channelElement = read(graphInputFilename);
			...
			if (channelElement instanceof VertexChannelElement<?>) {
				vertexes.add(((VertexChannelElement<GraphVertex>) channelElement).getObject());
			}

			if (channelElement instanceof EdgeChannelElement<?>) {
				edges.add(((EdgeChannelElement<GraphEdge>) channelElement).getObject());
			}
		}
	...
	{	
		...
		{
			write(new MRChannelElement<Integer, Double>(entry.getKey(), entry.getValue()), output);
		}
	}
